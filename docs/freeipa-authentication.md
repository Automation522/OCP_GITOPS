...
7. [Intégration ArgoCD](#intégration-argocd)

## Prérequis

| Composant | Valeur |
|-----------|--------|
| Serveur FreeIPA | `idm.skyr.dca.scc` |
| Port LDAPS | 636 |
| Base DN | `dc=skyr,dc=dca,dc=scc` |
| Users DN | `cn=users,cn=accounts,dc=skyr,dc=dca,dc=scc` |
| Bind DN | `uid=openshift,cn=users,cn=accounts,dc=skyr,dc=dca,dc=scc` |
...
|-----------|--------|-------------|
| `url` | `ldaps://idm.skyr.dca.scc:636/...?uid` | URL LDAP avec base de recherche et attribut de login |
| `bindDN` | `uid=openshift,...` | DN du compte de service pour interroger LDAP |
| `attributes.preferredUsername` | `uid` | Attribut utilisé comme nom d'utilisateur OpenShift |
| `attributes.name` | `cn` | Attribut utilisé comme nom complet |
| `attributes.email` | `mail` | Attribut utilisé comme email |
| `attributes.id` | `dn` | Attribut utilisé comme identifiant unique |
...
Cette section décrit la configuration de l'authentification FreeIPA pour les instances ArgoCD via Dex.
...
┌─────────────────┐      LDAPS:636      ┌─────────────────┐
│     ArgoCD      │ ──────────────────► │    FreeIPA      │
│   Dex Server    │                     │  idm.skyr.dca   │
└─────────────────┘                     └─────────────────┘
```

### Configuration

Pour chaque instance ArgoCD (par exemple `argocd-rbac`, `argocd-dev`, `argocd-gov`), ArgoCD utilise Dex comme courtier d'identité.

#### Configuration Dex

La configuration est définie dans la spec de la ressource `ArgoCD`. Il est recommandé d'utiliser `rootCAData` pour éviter les problèmes de montage de volume.
...
- [Documentation OpenShift - Configuring LDAP identity provider](https://docs.openshift.com/container-platform/4.18/authentication/identity_providers/configuring-ldap-identity-provider.html)
- [Documentation FreeIPA](https://www.freeipa.org/page/Documentation)
