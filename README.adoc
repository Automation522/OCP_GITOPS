...
* OpenShift 4.18 en mode air-gap avec stockage ODF (PVC RWX)
* FreeIPA pour l’authentification des comptes opérationnels (`demoscc`, etc.)
* Gitea interne (`bastion.skyr.dca.scc:3000`) comme dépôt Git suivi par Argo CD
* Harbor (`harbor.skyr.dca.scc`) comme registre miroir pour images applicatives et signatures
* Tekton Pipelines + Tekton Chains pour la CI et la signature cosign
* Argo CD (OpenShift GitOps) pour la synchronisation continue des manifestes
...
. Appliquer le manifest RBAC unique (modifier `manifests/base/rbac-access.yaml` pour pointer vers votre utilisateur si nécessaire) :
...
. Associer le ServiceAccount `postgres-db` (déployé automatiquement) à un SCC autorisant l'UID 26, par exemple `anyuid` :
...
----
. Appliquer la configuration Tekton Chains (namespace `openshift-pipelines`) :

[source,bash]
----
oc apply -f tekton/chains-config.yaml -n openshift-pipelines
----
. Astuce : appliquer d'un bloc tous les manifestes Tekton du namespace `gitops-demo` (hors PipelineRun) :
...
TIP: Si vous utilisez un autre compte que `demoscc`, adaptez `manifests/base/rbac-access.yaml` ou exécutez `oc adm policy add-role-to-user tekton-configmap-manager <user> -n openshift-pipelines` avant cette étape.

TIP: L'image PostgreSQL officielle tourne avec l'UID/GID 26. Sans SCC `anyuid` (ou équivalent) assigné au ServiceAccount `postgres-db`, le StatefulSet `postgres` échouera avec `Forbidden: unable to validate against any security context constraint`.
...
| Application web
| `build-local (sudo podman --runtime runc build ./app)`
| `harbor.skyr.dca.scc/gitops/customer-web:0.1.0`

| Job init PostgreSQL
| `build-local (sudo podman --runtime runc build ./app/db)`
...
[source,bash]
----
# Builder et pousser les images applicatives à partir de ce dépôt (rootful + runtime runc)
...
NOTE: Sur la bastion RHEL9, Podman rootless s'appuie sur `crun` (spec OCI 1.0) et échoue sur les `RUN` (`unknown version specified`). Exécutez les builds avec `sudo podman --runtime runc ...` ou mettez à jour l'OCI runtime si vous préférez rester en rootless.
