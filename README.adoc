...
. Appliquer le manifest RBAC unique (modifier `argocd/rbac-access.yaml` pour pointer vers votre utilisateur si nécessaire) :
...
TIP: Si vous utilisez un autre compte que `demoscc`, adaptez `argocd/rbac-access.yaml` ou exécutez `oc adm policy add-role-to-user tekton-configmap-manager <user> -n openshift-pipelines` avant cette étape.
