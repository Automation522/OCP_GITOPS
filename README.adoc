...
Le script applique les manifests RBAC (`argocd/role-argo-admin.yaml`, `argocd/rolebinding-argo-admin.yaml`) puis vérifie les droits via `oc auth can-i --as=demoscc`. Ajustez `ARGOCD_USER`/`ARGOCD_NAMESPACE` selon votre contexte.
